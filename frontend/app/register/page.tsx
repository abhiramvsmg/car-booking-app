...
import { UserPlus, Mail, Lock, User, Car, ShieldCheck } from 'lucide-react';
...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await fetcher.post('/auth/register', {
                email,
                password,
                full_name: fullName,
                role: role,
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-slate-950 flex items-center justify-center p-4 text-slate-900 dark:text-white transition-colors duration-300">
            <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl shadow-xl dark:shadow-2xl overflow-hidden p-8">
                <div className="flex flex-col items-center mb-8">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-2 font-black uppercase tracking-widest text-[10px]">Join the Elite Transport Fleet</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 mb-2">
                        <button
                            ...
                            className={`py-3 px-4 rounded-xl border flex flex-col items-center gap-2 transition-all ${role === 'rider'
                                ? 'bg-orange-50 dark:bg-orange-900/30 border-primary dark:border-primary text-primary dark:text-primary shadow-sm'
                                : 'bg-gray-50 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:border-gray-300 dark:hover:border-slate-600'
                                }`}
                        >
                            <User size={20} />
                            <span className="text-xs font-bold uppercase tracking-wider">Rider</span>
                        </button>
                        <button
                            type="button"
                            onClick={() => setRole('driver')}
                            className={`py-3 px-4 rounded-xl border flex flex-col items-center gap-2 transition-all ${role === 'driver'
                                ? 'bg-orange-50 dark:bg-orange-900/30 border-primary dark:border-primary text-primary dark:text-primary shadow-sm'
                                : 'bg-gray-50 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:border-gray-300 dark:hover:border-slate-600'
                                }`}
                        >
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                required
                                className="block w-full pl-10 pr-3 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all font-medium"
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-2 mt-6 py-4 bg-primary hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed text-white font-black uppercase tracking-widest text-xs rounded-xl shadow-lg shadow-primary/20 transition-all transform active:scale-[0.98]"
                    >
                        {loading ? (
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 dark:border-slate-800 text-center">
                    <p className="text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest">
                        ...
                        <Link href="/login" className="text-primary hover:text-orange-600 font-black transition-colors underline underline-offset-4">
                            Sign in to Fleet
                        </Link>
                    </p>
                </div>
